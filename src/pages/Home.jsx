    ...
    <div className="pb-12 bg-brand-cream animate-fade-in">
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 md:px-12 space-y-16">
        <div className="text-center space-y-2">
          <span className="text-brand-gold-dark text-xs tracking-widest uppercase block">— Curated Travel —</span>
          <h2 className="font-cormorant text-3xl md:text-4xl text-brand-dark tracking-wide uppercase">Journey Sectors</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="relative overflow-hidden aspect-[4/3] group cursor-pointer border border-brand-gold/10" onClick={() => setActiveTab('destinations', 'cultural-tourism')}>
            ...
            <img src={bayonBuddhaClose} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Cultural" />
            <div className="absolute inset-0 bg-brand-dark/55 flex flex-col justify-end p-6">
              <h3 className="font-serif text-base text-white uppercase">Cultural Tourism</h3>
              <p className="text-[12px] font-light text-brand-cream-dark/85 mt-1 leading-normal">Temples, ancient ruins, classical dance performances, and artisan crafts.</p>
            </div>
          </div>
          ...
          <div className="relative overflow-hidden aspect-[4/3] group cursor-pointer border border-brand-gold/10" onClick={() => setActiveTab('destinations', 'dark-tourism')}>
            ...
            <img src={darkSkulls} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Dark" />
            <div className="absolute inset-0 bg-brand-dark/65 flex flex-col justify-end p-6">
              <h3 className="font-serif text-base text-white uppercase">Dark Tourism</h3>
              <p className="text-[11px] font-light text-brand-cream-dark/85 mt-1 leading-normal">Reflective visits detailing history, resilience, and memory.</p>
            </div>
          </div>
          ...
          <div className="relative overflow-hidden aspect-[4/3] group cursor-pointer border border-brand-gold/10" onClick={() => setActiveTab('destinations', 'eco-tourism')}>
            ...
            <img src={yeakLaom} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Eco" />
            <div className="absolute inset-0 bg-brand-forest/65 flex flex-col justify-end p-6">
              <h3 className="font-serif text-base text-white uppercase">Eco Tourism</h3>
              <p className="text-[11px] font-light text-brand-cream-dark/85 mt-1 leading-normal">Volcanic crater lakes, rainforest conservation, and eco-lodges.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 md:px-12 space-y-16">
        <h2 className="text-center font-cormorant text-4xl tracking-widest text-brand-dark uppercase">Wonders Captured</h2>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          >
            {slideshowImages.map((slide, idx) => (
              ...
              >
                <img src={slide.src} alt={slide.label} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-brand-dark/90 via-brand-dark/50 to-transparent p-6">
                  <span className="text-[10px] tracking-[0.2em] font-semibold text-brand-gold-light uppercase block">Historic Landmark</span>
                  <span className="font-serif text-lg text-white font-light mt-1 block">{slide.label}</span>
                </div>
              </div>
            ))}
            ...
            <div className="absolute top-4 right-4 z-20 flex space-x-2 bg-brand-dark/40 backdrop-blur-xs px-3 py-1.5 rounded-none">
              {slideshowImages.map((_, idx) => (
                <div key={idx} className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${idx === slideIndex ? 'bg-brand-gold scale-125' : 'bg-white/40'}`} />
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="rounded-none overflow-hidden h-52 border border-brand-gold/10 group cursor-pointer" onClick={() => setActiveTab('gallery')}>
              <img src={apsaraDancer} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt="Apsara" />
            </div>
            <div className="rounded-none overflow-hidden h-52 border border-brand-gold/10 group cursor-pointer" onClick={() => setActiveTab('gallery')}>
              <img src={cambodianCulinary} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt="Culinary" />
            </div>
            <div className="sm:col-span-2 rounded-none overflow-hidden h-[216px] border border-brand-gold/10 group cursor-pointer" onClick={() => setActiveTab('gallery')}>
              <img src={kohRongBeach} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt="Coast" />
            </div>
          </div>
